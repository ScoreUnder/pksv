...
char FileOpen[MAX_PATH], FileOpen2[MAX_PATH + 40], FileOpen3[MAX_PATH],
    RomOpen[MAX_PATH], RomOpen2[MAX_PATH], SuperBuffer[MAX_PATH * 4],
    GreatBuffer[MAX_PATH * 2], DirBuffer[MAX_PATH * 2], DefRomPath[MAX_PATH],
    DefScriptPath[MAX_PATH], MegaBuffer[10240];
char AOT = 1, autoautocomplete = 0, FlagsShown = 0;
HWND MainWnd, StatusBar, Text, FindReplaceWin, GotoWin, FFSWin, DecWin, GenWin,
    InputWin, Input2Win, NoteWin, ToolBar, HexWin, MovePlanWin, FlagsWin,
    ScriptSearchWin, LockWin
#ifdef SCRAP
    ,
    PickupWin, ScrapPunk
#endif
...
BOOL CALLBACK FindReplaceFunc(HWND hwnd, UINT msg, WPARAM wParam,
                              LPARAM lParam);
...
BOOL CALLBACK ScriptSearchFunc(HWND hwnd, UINT msg, WPARAM wParam,
                               LPARAM lParam);
...
void SaveRegSetting(char *sub, char *set, int val) {
  ...
  RegCreateKeyEx(HKEY_CURRENT_USER, subbuf, 0, NULL, REG_OPTION_NON_VOLATILE,
                 KEY_WRITE, NULL, &regkey, &dummy);
...
#define get_update                                                      \
  ...
  "pk-script-view.sourceforge.net\nAccept-Encoding: None\nConnection: " \
  "Close\n\n"
        ...
        if (pos + 3 <= sl_sock && memcmp(&buffer[pos], "200", 3) == 0) {
          ...
          if (!same_version) {
            ...
            if (DialogBox(inst, MAKEINTRESOURCE(3077), MainWnd,
                          UpdatesDlgProc)) {
...
#define URL_BASE_LEN (sizeof(URL_BASE) - 1)
              ...
              strcpy(&UpdateBuffer[URL_BASE_LEN + ver_len],
                     ".zip?use_mirror=osdn");
              ShellExecute(GenWin, NULL, UpdateBuffer, NULL, NULL,
                           SW_SHOWDEFAULT);
            }
          } else if (explicit_update_check) {
            DialogBox(inst, MAKEINTRESOURCE(3076), MainWnd,
                      UpdatesDlgProc);  // Dialog, NO UPDATE AVAILABLE
          }
        }
      }
    }
...
BOOL CALLBACK UpdatesDlgProc(HWND h, UINT m, WPARAM w, LPARAM l) {
  ...
  switch (m) {
    ...
    case WM_COMMAND:
      switch (LOWORD(w)) {
        case 1:
          SaveRegSetting(NULL, "AutoUpdate",
                         IsDlgButtonChecked(h, 1) == BST_CHECKED);
          break;
        case 2:
          EndDialog(h, 1);
          break;
...
int foundfont = 0;
// FONTENUMARATOR
int CALLBACK FontEnumerator(ENUMLOGFONT *enf, NEWTEXTMETRIC *z, int x,
                            LPARAM c) {
  UNUSED_ARG(z);
  UNUSED_ARG(x);
  UNUSED_ARG(c);
  ToLower((char *)enf->elfFullName);
  if (!strcmp((char *)enf->elfFullName, "lucida console")) foundfont = 1;
  return 1;
}
...
void DeleteLastFlag(void) {
  ...
  for (i = LastFlag; i < FlagArrLen; i++) FlagArr[i] = FlagArr[i + 1];
  LastFlag = -1;
...
void ReAddFlags(void) {
  char buf[5];
  register int i;
  FlagArrLen = (FlagArrAlloc = LoadRegSettingBin("Flags", "UsedFlags",
                                                 (char **)&FlagArr)) >>
               1;
  SortFlags();
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
  ...
  VersionMismatch =
      (int(__cdecl *)(char *))GetProcAddress(PKSV, "VersionMismatch");
  ...
  decompile =
      (char *(__cdecl *)(char *, int, int))GetProcAddress(PKSV, "decompile");
  decompileASM =
      (char *(__cdecl *)(char *, int))GetProcAddress(PKSV, "decompileASM");
  decompileLevel =
      (char *(__cdecl *)(char *, int))GetProcAddress(PKSV, "decompileLevel");
  decompileText =
      (char *(__cdecl *)(char *, int))GetProcAddress(PKSV, "decompileText");
  decompileMart =
      (char *(__cdecl *)(char *, int))GetProcAddress(PKSV, "decompileMart");
  decompileMoves =
      (char *(__cdecl *)(char *, int))GetProcAddress(PKSV, "decompileMoves");
  decompilePointer =
      (char *(__cdecl *)(char *, int))GetProcAddress(PKSV, "decompilePointer");
  decompileAttacks =
      (char *(__cdecl *)(char *, int))GetProcAddress(PKSV, "decompileAttacks");
  NewMode = (char *(__cdecl *)(int))GetProcAddress(PKSV, "NewMode");
  compile = (int(__cdecl *)(char *, char *))GetProcAddress(PKSV, "compile");
  DebugCompile =
      (int(__cdecl *)(char *, char *))GetProcAddress(PKSV, "DebugCompile");
  SetDynamic = (int(__cdecl *)(int, int))GetProcAddress(PKSV, "SetDynamic");
  if (!VersionMismatch || !NewMode || !DetermineMode || !decompile ||
      !decompileASM || !decompileLevel || !decompileText || !compile ||
      !decompileMart || !decompileMoves || !decompilePointer ||
      !decompileAttacks || !SetDynamic) {
    MessageBox(NULL, GetString1(3069), GetString2(3001), 0x10);
    // Failed to load PKSV.DLL or failed to load one of PKSV.DLL's functions.
    return 0;
  }
  ...
  CreateListClass =
      (int(__cdecl *)())GetProcAddress(SearchBox, "CreateListClass");
  if (!CreateListClass) {
    MessageBox(NULL, "Failed to load a function in searchbox.dll",
               GetString2(3001), 0x10);
    return 0;
  }
  CreateListClass();
#endif
  ...
  *RomOpen2 = 0;
  ...
  // Settings
  RegOpenKeyEx(HKEY_CURRENT_USER, "Software\\Score_Under\\PKSV-UI", 0, KEY_READ,
               &regkey);
  DataSize = sizeof(Data);
  Data = 0;
  RegQueryValueEx(regkey, "CollapseBeforeEnd", NULL, (LPDWORD)&Nothing,
  ...
  if (Data == 0)
    ...
    collapseatend = 0;
  Data = 0;
  RegQueryValueEx(regkey, "CollapseEmptyLines", NULL, (LPDWORD)&Nothing,
                  (LPBYTE)&Data, (LPDWORD)&DataSize);
  if (Data != 0)
    collapselines = 1;
  else
    collapselines = 0;
  Data = 0;
  RegQueryValueEx(regkey, "AutoAutoComplete", NULL, (LPDWORD)&Nothing,
                  (LPBYTE)&Data, (LPDWORD)&DataSize);
  if (Data != 0)
    autoautocomplete = 1;
  else
  ...
  MainWnd = CreateWindow(
      "Score_Under_PKSVUI", "", WS_OVERLAPPEDWINDOW | WS_POPUPWINDOW,
      ...
      WinSize.bottom + GetSystemMetrics(SM_CYCAPTION) +
          GetSystemMetrics(SM_CYFRAME),
      HWND_DESKTOP, NULL, hInstance, NULL);
  StatusBar = CreateWindow(STATUSCLASSNAME, "Status Bar", WS_CHILD | WS_VISIBLE,
                           0, 0, 0, 0, MainWnd, NULL, hInstance, NULL);
#ifdef SCRAP
  Text = CreateWindowEx(
      WS_EX_CLIENTEDGE, "Scintilla", "",
      WS_CHILD | WS_VISIBLE | ES_LEFT | ES_MULTILINE | ES_NOHIDESEL, 0, 24,
      WIDTH - 32, HEIGHT - 67, MainWnd, NULL, hInstance, NULL);
  ScrapPunk =
      CreateWindow("Button", "", WS_CHILD | WS_VISIBLE | BS_OWNERDRAW,
                   WIDTH - 32, 24, 32, 32, MainWnd, NULL, hInstance, NULL);
#else
  Text = CreateWindowEx(
      WS_EX_CLIENTEDGE, "Scintilla", "",
      WS_CHILD | WS_VISIBLE | ES_LEFT | ES_MULTILINE | ES_NOHIDESEL, 0, 24,
      WIDTH, HEIGHT - 67, MainWnd, NULL, hInstance, NULL);
#endif
  InitCommonControls();
  dc = GetDC(MainWnd);
  if (GetDeviceCaps(dc, BITSPIXEL) < 32)
    ToolBar = CreateToolbarEx(
        MainWnd,
        WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | CCS_ADJUSTABLE |
            CCS_NODIVIDER | CCS_NOMOVEY | TBSTYLE_TOOLTIPS,
        5000, 20, inst, 6, ToolButtons, sizeof(ToolButtons) / sizeof(TBBUTTON),
    ...
    ToolBar = CreateToolbarEx(
        ...
        WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | CCS_ADJUSTABLE |
            CCS_NODIVIDER | CCS_NOMOVEY | TBSTYLE_TOOLTIPS,
        5000, 20, inst, 5, ToolButtons, sizeof(ToolButtons) / sizeof(TBBUTTON),
        16, 16, 16, 16, sizeof(TBBUTTON));
  ...
  FindReplaceWin = CreateDialog(inst, MAKEINTRESOURCE(100), MainWnd,
                                (DLGPROC)&FindReplaceFunc);
  if (FindReplaceWin == NULL) {
    FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, NULL, GetLastError(), 0, tch,
                  sizeof(tch), NULL);
    MessageBox(MainWnd, tch, GetString2(3001), 0x10);
  }
  GotoWin =
      CreateDialog(inst, MAKEINTRESOURCE(101), MainWnd, (DLGPROC)&GotoFunc);
  if (GotoWin == NULL) {
    FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, NULL, GetLastError(), 0, tch,
                  sizeof(tch), NULL);
    MessageBox(MainWnd, tch, GetString2(3001), 0x10);
  ...
  InputWin =
      CreateDialog(inst, MAKEINTRESOURCE(105), MainWnd, (DLGPROC)&InputFunc);
  if (InputWin == NULL) {
    FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, NULL, GetLastError(), 0, tch,
                  sizeof(tch), NULL);
    MessageBox(MainWnd, tch, GetString2(3001), 0x10);
  ...
  Input2Win =
      CreateDialog(inst, MAKEINTRESOURCE(3070), MainWnd, (DLGPROC)&Input2Func);
  if (Input2Win == NULL) {
    FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, NULL, GetLastError(), 0, tch,
                  sizeof(tch), NULL);
    MessageBox(MainWnd, tch, GetString2(3001), 0x10);
  ...
  NoteWin =
      CreateDialog(inst, MAKEINTRESOURCE(106), MainWnd, (DLGPROC)&NoteFunc);
  if (NoteWin == NULL) {
    FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, NULL, GetLastError(), 0, tch,
                  sizeof(tch), NULL);
    MessageBox(MainWnd, tch, GetString2(3001), 0x10);
  ...
  MovePlanWin =
      CreateDialog(inst, MAKEINTRESOURCE(108), MainWnd, (DLGPROC)&MovePlanFunc);
  if (MovePlanWin == NULL) {
    FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, NULL, GetLastError(), 0, tch,
                  sizeof(tch), NULL);
    MessageBox(MainWnd, tch, GetString2(3001), 0x10);
  }
  ScriptSearchWin = CreateDialog(inst, MAKEINTRESOURCE(109), MainWnd,
                                 (DLGPROC)&ScriptSearchFunc);
  ...
  FlagsWin =
      CreateDialog(inst, MAKEINTRESOURCE(3071), MainWnd, (DLGPROC)&FlagsFunc);
  if (FlagsWin == NULL) {
    FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, NULL, GetLastError(), 0, tch,
                  sizeof(tch), NULL);
    MessageBox(MainWnd, tch, GetString2(3001), 0x10);
  ...
  LockWin =
      CreateDialog(inst, MAKEINTRESOURCE(3072), MainWnd, (DLGPROC)&LockFunc);
  if (LockWin == NULL) {
    FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, NULL, GetLastError(), 0, tch,
                  sizeof(tch), NULL);
    MessageBox(MainWnd, tch, GetString2(3001), 0x10);
...
#ifdef SCRAP
  PickupWin =
      CreateDialog(inst, MAKEINTRESOURCE(3073), MainWnd, (DLGPROC)&PickupFunc);
  if (PickupWin == NULL) {
    FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, NULL, GetLastError(), 0, tch,
                  sizeof(tch), NULL);
    MessageBox(MainWnd, tch, GetString2(3001), 0x10);
  ...
  SendMessage(PickupWin, WM_SETICON, 0,
              (LPARAM)LoadIcon(inst, MAKEINTRESOURCE(5)));
#endif
  ...
  SendMessage(StatusBar, WM_SETFONT, (LPARAM)GetStockObject(DEFAULT_GUI_FONT),
              0);
  ...
  SendEditor(SCI_MARKERDEFINE, SC_MARKNUM_FOLDEROPENMID,
  ...
  if (x[0] != 0) {
    ...
    if (y != 0 && !IsRom) {
      ...
      fileM = CreateFile(FileOpen, GENERIC_READ, FILE_SHARE_READ, NULL,
                         OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
      if (GetLastError() != 0) {
        FormatMessage(
            FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM, 0,
            GetLastError(), 0, (char *)&ptr, 2000, NULL);
      ...
      fileM = CreateFile(RomOpen, GENERIC_READ,
      ...
      if (fileM != INVALID_HANDLE_VALUE) {
        ...
        if ((determineMode[0] == 'A' && determineMode[1] == 'A') ||
            (determineMode[0] == 'S' &&
             determineMode[1] == 'M')) {  // Detect GSC and Prism
          pokemon_version = GBC;
        } else {
  ...
  while (GetMessage(&msg, NULL, 0, 0)) {
    if (!TranslateAccelerator(MainWnd, hAccel, &msg)) {
      if (!IsDialogMessage(FFSWin, &msg) &&
          ...
          !IsDialogMessage(GotoWin, &msg) && !IsDialogMessage(NoteWin, &msg) &&
          !IsDialogMessage(MovePlanWin, &msg) &&
          !IsDialogMessage(HexWin, &msg) && !IsDialogMessage(InputWin, &msg) &&
          !IsDialogMessage(Input2Win, &msg) && !IsDialogMessage(GenWin, &msg) &&
          !IsDialogMessage(DecWin, &msg) &&
          !IsDialogMessage(ScriptSearchWin, &msg) &&
          !IsDialogMessage(LockWin, &msg) && !IsDialogMessage(FlagsWin, &msg)
#ifdef SCRAP
...
#define StrDefault GetString1(3016)
  switch (msg) {
      ...
      if (hwnd == MainWnd) {
        switch (nmh->code) {
          ...
          case TBN_GETBUTTONINFO:
            if (((TBNOTIFY *)nmh)->iItem >= 0 &&
                (size_t)((TBNOTIFY *)nmh)->iItem <
                    (sizeof(ToolButtons) / sizeof(TBBUTTON))) {
              ((TBNOTIFY *)nmh)->tbButton =
                  ToolButtons[((TBNOTIFY *)nmh)->iItem];
              return 1;
            }
            return 0;
          case SCN_MARGINCLICK:
            ...
            SendEditor(SCI_TOGGLEFOLD,
                       SendEditor(SCI_LINEFROMPOSITION, scn->position, 0), 0);
            break;
          case SCN_CHARADDED:
            if (autoautocomplete &&
                ((((struct SCNotification *)nmh)->ch) != '\n') &&
                ((((struct SCNotification *)nmh)->ch) != '\r')) {
              len = 0;
              i = (int)SendEditor(SCI_GETSELECTIONSTART, 0, 0);
              start =
                  (int)SendEditor(SCI_POSITIONFROMLINE,
                                  SendEditor(SCI_LINEFROMPOSITION, i, 0), 0);
              while (SciCharAt(start) == ' ') start++;
              while (i > start) {
                if (SciCharAt(i) == '\'' ||
                    (SciCharAt(i) == '/' && SciCharAt(i + 1) == '/')) {
                  break;
                }
                len++;
                i--;
            ...
            while (sges != (unsigned int)SendEditor(SCI_GETENDSTYLED, 0, 0)) {
              ...
              line =
                  (int)SendEditor(SCI_LINEFROMPOSITION,
                                  SendMessage(Text, SCI_GETENDSTYLED, 0, 0), 0);
              ...
              while (start < (ostart + len) &&
                     (SciCharAt(start) == ' ' || SciCharAt(start) == '\t')) {
                start++;
              }
              SendEditor(SCI_STARTSTYLING, start, 0x1f);
              if (SendEditor(SCI_GETCHARAT, start, 0) == '#') {
                SendEditor(SCI_SETSTYLING, len - (start - ostart),
                           STYLE_SPECIAL);
                if ((cToLower(SciCharAt(start + 1)) == 'o' &&
                     cToLower(SciCharAt(start + 2)) == 'r' &&
                     cToLower(SciCharAt(start + 3)) == 'g' &&
                     cToLower(SciCharAt(start + 4)) == ' ') ||
                    (cToLower(SciCharAt(start + 1)) == 'e' &&
                     cToLower(SciCharAt(start + 2)) == 'o' &&
                     cToLower(SciCharAt(start + 3)) == 'r' &&
                     cToLower(SciCharAt(start + 4)) == 'g' &&
                     cToLower(SciCharAt(start + 5)) == ' ')) {
                  SendEditor(SCI_SETFOLDLEVEL, line,
                             SC_FOLDLEVELHEADERFLAG | 1024);
                } else {
                  ...
                  i = 0;
                  while ((i + start2) < ostart &&
                         SciCharAt(start2 + i) != ' ' &&
                         SciCharAt(start2 + i) != '\'' &&
                         (SciCharAt(start2 + i) != '/' ||
                          SciCharAt(start2 + i + 1) != '/') &&
                         SciCharAt(start2 + i) != '\n' &&
                  ...
                  lbuf[i] = 0;
                  ToLower(lbuf);
                  if (!strcmp(lbuf, "end") || !strcmp(lbuf, "jump") ||
                      !strcmp(lbuf, "jumpstd") || !strcmp(lbuf, "goto") ||
                      !strcmp(lbuf, "return") || !strcmp(lbuf, "m") ||
                      !strcmp(lbuf, "=")) {
                    SendEditor(SCI_SETFOLDLEVEL, line,
                               1024);  // Don't fold blank lines at end.
                  } else if (line == 0 || SendEditor(SCI_GETFOLDLEVEL, line - 1,
                                                     0) == 1024) {
                ...
                GlobalFree(lbuf);
                break;
              } else if (SciCharAt(start) == '=' || SciCharAt(start) == '.' ||
                         (cToLower(SciCharAt(start)) == 'm' &&
                          SciCharAt(start + 1) == ' ')) {
                SendEditor(SCI_SETSTYLING, 1, STYLE_ARGUMENT);
                SendEditor(SCI_STARTSTYLING, start + 1, 0x1f);
                SendEditor(SCI_SETSTYLING, (len - 1) - (start - ostart),
                           STYLE_MESSAGE);
                while (SciCharAt(ostart + len - 1) == '\\') {
                  ostart += len;
                  len = SendEditor(SCI_LINELENGTH,
                                   SendEditor(SCI_LINEFROMPOSITION, ostart, 0),
                                   0);
                  SendEditor(SCI_SETSTYLING, len, STYLE_MESSAGE);
                  SendEditor(SCI_SETFOLDLEVEL,
                             SendEditor(SCI_LINEFROMPOSITION, ostart, 0), 1025);
                }
                SendEditor(SCI_SETFOLDLEVEL, line, 1025);
                GlobalFree(lbuf);
                break;
                ...
                if (line == 0) {
                  ...
                  dontsetlevel = 1;
                           ...
                           (SciCharAt(start) == '/' &&
                            SciCharAt(start + 1) == '/') ||
                           SendEditor(SCI_GETFOLDLEVEL, line - 1, 0) == 1024) {
                  if (SendEditor(SCI_GETFOLDLEVEL, line - 1, 0) == 1024 &&
                      !dontsetlevel) {
                    SendEditor(SCI_SETFOLDLEVEL, line, 1024);
                  ...
                  if (!dontsetlevel) {
                    if (collapseatend) {
                      ...
                      SendEditor(SCI_SETFOLDLEVEL, line, 1024);
                ...
                } else if (!strcmp(lbuf, "lock") ||
                           !strcmp(lbuf, "faceplayer") ||
                           !strcmp(lbuf, "release") ||
                           !strcmp(lbuf, "msgbox") ||
                           !strcmp(lbuf, "callstd") ||
                           !strcmp(lbuf, "callasm") ||
                           !strcmp(lbuf, "message") ||
                           !strcmp(lbuf, "showmsg") ||
                           !strcmp(lbuf, "closemsg") ||
                           !strcmp(lbuf, "releaseall") ||
                           !strcmp(lbuf, "lockall") ||
                           !strcmp(lbuf, "waitbutton") || !strcmp(lbuf, "if")) {
                  SendEditor(SCI_STARTSTYLING, start, 0x1f);
                  SendEditor(SCI_SETSTYLING, len - (start - ostart),
                             STYLE_IMPORTANT);
              ...
              while (SciCharAt(start) != '\'' &&
                     (SciCharAt(start) != '/' || SciCharAt(start + 1) != '/') &&
                     SciCharAt(start) != '\n' && SciCharAt(start) != '\r' &&
                     SciCharAt(start) != ' ' && SciCharAt(start) != '\t' &&
                     start < (ostart + len)) {
                start++;
    ...
    case WM_DROPFILES:
      DragQueryFile(
          (HANDLE)wParam, 0,
          lbuf = alloca(DragQueryFile((HANDLE)wParam, 0, NULL, 0) + 1),
          sizeof(GreatBuffer));
      ...
      if (fileM != INVALID_HANDLE_VALUE) {
        CloseHandle(fileM);
        i = strlen(lbuf);
        if (!strcmp(lbuf + i - 4, ".gba") || !strcmp(lbuf + i - 4, ".nds") ||
            !strcmp(lbuf + i - 4, ".gbc") || !strcmp(lbuf + i - 5, ".narc") ||
            !strcmp(lbuf + i - 3, ".gb")) {
          strcpy(RomOpen, lbuf);
          goto OpenRoms;
        } else {
    ...
    case WM_COMMAND:
      ...
      if (lParam == (LPARAM)ScrapPunk) {
        ...
        switch (LOWORD(wParam)) {
          ...
          case 1007:
            if (NotSaved) {
              i = MessageBox(MainWnd, GetString1(3017), GetString2(3018),
          ...
          case ID_NEW:
            i = 0;
            if (NotSaved) {
              i = MessageBox(MainWnd, GetString1(3017), GetString2(3019),
                             MB_YESNOCANCEL | MB_ICONQUESTION);
              if (i == 6) {
                SendMessage(MainWnd, WM_COMMAND, ID_SAVE, 0);
                if (NotSaved) {
          ...
          case ID_OPEN:
            ...
            ofn.lpstrFilter =
                "PKSV Scripts\0*.pks\0PKSV Temp\0_?_.pks.tmp\0Rubikon/XSE "
                "script (*.rbc;*.rbh)\0*.rbc;*.rbh\0Text Files\0*.txt\0All "
                "Files\0*.*\0";
            ...
            if (*DefScriptPath != 0) SetCurrentDirectory(DefScriptPath);
            if (GetOpenFileName(&ofn)) {
              if (cToLower(FileOpen[strlen(FileOpen) - 3]) == 'g' &&
                  cToLower(FileOpen[strlen(FileOpen) - 2]) == 'b' &&
                  cToLower(FileOpen[strlen(FileOpen) - 1]) == 'a') {
                MessageBox(MainWnd,
                           "Warning: This appears to be a GBA ROM, yet you "
                           "have opened it with the .PKS script "
                           "opener.\nChoose Rom Tools->Open ROM for the "
                           "expected behaviour if this was a ROM.",
                           "Warning", 0x30);
              }
            OpenMe:
              strcpy(FileOpen2, FileOpen);
              ...
              if (GetLastError() != 0) {
                FormatMessage(
                    ...
                    0, GetLastError(), 0, (char *)&ptr, 2000, NULL);
                MessageBox(MainWnd, ptr, ptr, 0x10);
                LocalFree(ptr);
                return 0;
              }
          ...
          case ID_SAVEAS:
            ...
            ofn.hInstance = GetModuleHandle(NULL);
            ofn.lpstrFilter =
                "PKSV Scripts\0*.pks\0Text Files\0*.txt\0All Files\0*.*\0";
            ofn.lpstrFile = FileOpen;
            LoadRegSettingStr(NULL, "ScriptPath");
            strcpy(DefScriptPath, GreatBuffer);
            ofn.lpstrInitialDir = DefScriptPath;
            ...
            if (GetSaveFileName(&ofn) && strcmp(ofn.lpstrFile, "")) {
              ...
              SetWindowText(MainWnd, FileOpen2);
              FileSave =
                  CreateFile(FileOpen3, GENERIC_WRITE, FILE_SHARE_READ, NULL,
              ...
              if (FileSave != INVALID_HANDLE_VALUE) {
                ...
                while ((int)read < i - start) {
                  WriteFile(FileSave, (void *)(((char *)ptr) + start),
                            i - start, &read, NULL);
                  start += read;
                  end--;
                  if (!end) {
                    ...
                    MessageBox(MainWnd, GetString1(3023), GetString2(3022),
                               0x40);
          ...
          case ID_SAVE:
            if (strcmp(FileOpen3, GetString1(3008))) {
              ...
              if (FileSave != INVALID_HANDLE_VALUE) {
                ...
                while ((int)read < (i - start)) {
                  WriteFile(FileSave, (char *)ptr + start, (i - start), &read,
                            NULL);
                  start += read;
                  end--;
                  if (!end) {
          ...
          case 1020:
            ...
            SendEditor(SCI_COLOURISE, 0, (LPARAM)-1);
            RegCreateKeyEx(HKEY_CURRENT_USER, "Software\\Score_Under\\PKSV-UI",
                           0, NULL, REG_OPTION_NON_VOLATILE, KEY_WRITE, NULL,
                           &regkey, (DWORD *)&start);
            i = 1 - collapseatend;
            RegSetValueEx(regkey, "CollapseBeforeEnd", 0, REG_DWORD,
                          (CONST BYTE *)&i, 4);
            RegCloseKey(regkey);
            break;
          case 1021:
            ...
            RegCreateKeyEx(HKEY_CURRENT_USER, "Software\\Score_Under\\PKSV-UI",
                           ...
                           &regkey, (DWORD *)&start);
            i = collapselines;
            RegSetValueEx(regkey, "CollapseEmptyLines", 0, REG_DWORD,
                          (CONST BYTE *)&i, 4);
            RegCloseKey(regkey);
            break;
          case 1112:
            ...
            RegCreateKeyEx(HKEY_CURRENT_USER, "Software\\Score_Under\\PKSV-UI",
                           ...
                           &regkey, (DWORD *)&start);
            i = autoautocomplete;
            RegSetValueEx(regkey, "AutoAutoComplete", 0, REG_DWORD,
                          (CONST BYTE *)&i, 4);
            RegCloseKey(regkey);
            break;
          case 1012:
          ...
          case 1018:
            ...
            sprintf(buf, "%lu",
                    SendEditor(SCI_LINEFROMPOSITION,
                               SendEditor(SCI_GETSELECTIONSTART, 0, 0), 0));
            SetDlgItemText(GotoWin, 3, buf);
            sprintf(
                buf, "%lu",
                SendEditor(SCI_GETSELECTIONSTART, 0, 0) -
                    SendEditor(
          ...
          case 1202:
            if (*RomOpen2 != 0) {
              ...
              CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)TDebugCompile,
                           (LPVOID)ptr, 0, (LPVOID)&i);
            } else {
...
#include "default_flags.h"
          ...
          case 1013:  // script-gen
            i = 0;
            if (NotSaved) {
              i = MessageBox(MainWnd, GetString1(3032), GetString2(3028),
                             MB_YESNOCANCEL | MB_ICONQUESTION);
              if (i == IDYES) {
                SendMessage(MainWnd, WM_COMMAND, ID_SAVE, 0);
                if (NotSaved) {
          ...
          case 1111:
            ...
            start = (int)SendEditor(SCI_POSITIONFROMLINE,
                                    SendEditor(SCI_LINEFROMPOSITION, i, 0), 0);
            while (SciCharAt(start) == ' ') start++;
            if (SciCharAt(start) == '=') {
              end = 1;
            }
            if ((SciCharAt(start) == 'm' || SciCharAt(start) == 'M') &&
                SciCharAt(start + 1) == ' ') {
              end = 3;
            }
...
checkattack;3|checkflag;6|checkifroomforfurniture;5|checkitem;4|\
...
teleport_from;1|teleport_to;1|tree_animate;1|waterfall_down;1|waterfall_left;1|waterfall_right;1|waterfall_up;1");
          ...
          case 1004:  // compile, decompile is 1014
            if (*RomOpen2 != 0) {
              ...
              SendEditor(SCI_GETTEXT, i + 1, (LPARAM)ptr);
              DetermineMode(RomOpen2);
              CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)TCompile,
                           (LPVOID)ptr, 0, (LPVOID)&i);
            } else {
              MessageBox(MainWnd, GetString1(3034), GetString2(3001), 0x40);
            }
...
#ifdef SCRAP
        SetWindowPos(Text, HWND_TOP, 0, 24, LOWORD(lParam) - 32,
                     HIWORD(lParam) - 47, SWP_NOMOVE);
        SetWindowPos(ScrapPunk, HWND_TOP, LOWORD(lParam) - 32, 24, 32, 32,
                     SWP_NOSIZE);
#else
        SetWindowPos(Text, HWND_TOP, 0, 24, LOWORD(lParam), HIWORD(lParam) - 47,
                     SWP_NOMOVE);
#endif
      } else {
        return DefWindowProc(hwnd, msg, wParam, lParam);
...
void GoToLine(unsigned int line_number, unsigned int pos) {
  ...
  if ((int)SendEditor(SCI_POSITIONFROMLINE, line_number + 1, 0) <=
      (int)(SendEditor(SCI_POSITIONFROMLINE, line_number, 0) + pos))
    SendEditor(SCI_GOTOPOS, SendEditor(SCI_LINELENGTH, line_number, 0), 0);
  else
    SendEditor(SCI_GOTOPOS,
               pos + SendEditor(SCI_POSITIONFROMLINE, line_number, 0), 0);
}
      ...
      switch (LOWORD(wParam)) {
        case 0x1:  // Find
          ...
          if (Upwards) {
            ...
            SendEditor(SCI_SETTARGETSTART,
                       SendEditor(SCI_GETSELECTIONEND, 0, 0), 0);
            SendEditor(SCI_SETTARGETEND, SendEditor(SCI_GETLENGTH, 0, 0), 0);
          }
          found = (int)SendEditor(SCI_SEARCHINTARGET, strlen(findbuf),
                                  (LPARAM)findbuf);
          if (found == -1) {
            MessageBox(FindReplaceWin, GetString1(3035), "PKSVUI", 0x40);
            if (Upwards) {
              SendEditor(SCI_SETSEARCHFLAGS, flags, 0);
              SendEditor(SCI_SETTARGETSTART, SendEditor(SCI_GETLENGTH, 0, 0),
                         0);
              SendEditor(SCI_SETTARGETEND, 0, 0);
              found = (int)SendEditor(SCI_SEARCHINTARGET, strlen(findbuf),
                                      (LPARAM)findbuf);
            ...
            if (found == -1) {
              ...
              SendEditor(SCI_SETSEL, SendEditor(SCI_GETTARGETSTART, 0, 0),
                         SendEditor(SCI_GETTARGETEND, 0, 0));
            }
          } else {
            SendEditor(SCI_SETSEL, SendEditor(SCI_GETTARGETSTART, 0, 0),
                       SendEditor(SCI_GETTARGETEND, 0, 0));
        ...
        case 0x2:  // Replace
          ...
          if (selstart != selend) {
            if (RegExpr)
              SendEditor(SCI_REPLACETARGETRE, (WPARAM)-1, (LPARAM)repbuf);
            else
              SendEditor(SCI_REPLACETARGET, (WPARAM)-1, (LPARAM)repbuf);
          }
          break;
        ...
        case 0x4:  // Cancel
          ...
          break;
        case 0x9:  // Match case
          MatchCase = (IsDlgButtonChecked(FindReplaceWin, LOWORD(wParam)) ==
                       BST_CHECKED);
          break;
        case 0xA:  // Search up
          Upwards = (IsDlgButtonChecked(FindReplaceWin, LOWORD(wParam)) ==
                     BST_CHECKED);
          break;
        case 0xB:  // Regex
          RegExpr = (IsDlgButtonChecked(FindReplaceWin, LOWORD(wParam)) ==
                     BST_CHECKED);
          break;
      }
      break;
...
off_t find_free_space_raw(FILE *file, size_t required, uint8_t searchFor) {
  ...
  if (consecutive < required) return -1;
...
uint32_t FindFreeSpace(char *romname, uint32_t len, uint32_t start) {
  FILE *rom_file = windows_handle_as_stdio(
      CreateFile(romname, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE,
                 NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL),
      "rb");

  if (rom_file == NULL) return (uint32_t)-1;  // TODO: error message?
    ...
    case WM_COMMAND:
      switch (LOWORD(wParam)) {
        case 0x1:  // Decompile
          if (*RomOpen2 != 0) {
            if (NotSaved) {
              i = MessageBox(MainWnd, GetString1(3017), GetString2(3040),
                             MB_YESNOCANCEL | MB_ICONQUESTION);
              if (i == IDYES) {
                SendMessage(MainWnd, WM_COMMAND, ID_SAVE, 0);
                if (NotSaved) {
            ...
            if (block == (unsigned)-1) sscanf(GreatBuffer, "%X", &block);
            if (block == (unsigned)-1) sscanf(GreatBuffer, "$%X", &block);
            SaveRegSetting("Decompiler", "Use Dynamic",
                           IsDlgButtonChecked(DecWin, 18));
            ...
            SendEditor(SCI_MARKERDELETEALL, (WPARAM)-1, 0);
            SendEditor(SCI_SETSCROLLWIDTH, 2048, 0);
            if (ptr) SendEditor(SCI_ADDTEXT, strlen(ptr), (LPARAM)ptr);
            SendEditor(SCI_SETUNDOCOLLECTION, 1, 0);
            SendEditor(EM_EMPTYUNDOBUFFER, 0, 0);
            SendEditor(SCI_SETSAVEPOINT, 0, 0);
            SendEditor(SCI_GOTOPOS, 0, 0);
...
void FillItems(HWND hwndList) {
  char *pVarName[] = {
      "NONE",          "MASTERBALL",   "ULTRABALL",    "GREATBALL",
      "POKEBALL",      "SAFARIBALL",   "NETBALL",      "DIVEBALL",
      "NESTBALL",      "REPEATBALL",   "TIMERBALL",    "LUXURYBALL",
      "PREMIERBALL",   "POTION",       "ANTIDOTE",     "BURNHEAL",
      "ICEHEAL",       "AWAKENING",    "PARLYZHEAL",   "FULLRESTORE",
      "MAXPOTION",     "HYPERPOTION",  "SUPERPOTION",  "FULLHEAL",
      "REVIVE",        "MAXREVIVE",    "FRESHWATER",   "SODAPOP",
      "LEMONADE",      "MOOMOOMILK",   "ENERGYPOWDER", "ENERGYROOT",
      "HEALPOWDER",    "REVIVALHERB",  "ETHER",        "MAXETHER",
      "ELIXIR",        "MAXELIXIR",    "LAVACOOKIE",   "BLUEFLUTE",
      "YELLOWFLUTE",   "REDFLUTE",     "BLACKFLUTE",   "WHITEFLUTE",
      "BERRYJUICE",    "SACREDASH",    "SHOALSALT",    "SHOALSHELL",
      "REDSHARD",      "BLUESHARD",    "YELLOWSHARD",  "GREENSHARD",
      "HPUP",          "PROTEIN",      "IRON",         "CARBOS",
      "CALCIUM",       "RARECANDY",    "PPUP",         "ZINC",
      "PPMAX",         "GUARDSPEC",    "DIREHIT",      "XATTACK",
      "XDEFEND",       "XSPEED",       "XACCURACY",    "XSPECIAL",
      "POKEDOLL",      "FLUFFYTAIL",   "SUPERREPEL",   "MAXREPEL",
      "ESCAPEROPE",    "REPEL",        "SUNSTONE",     "MOONSTONE",
      "FIRESTONE",     "THUNDERSTONE", "WATERSTONE",   "LEAFSTONE",
      "TINYMUSHROOM",  "BIGMUSHROOM",  "PEARL",        "BIGPEARL",
      "STARDUST",      "STARPIECE",    "NUGGET",       "HEARTSCALE",
      "ORANGEMAIL",    "HARBORMAIL",   "GLITTERMAIL",  "MECHMAIL",
      "WOODMAIL",      "WAVEMAIL",     "BEADMAIL",     "SHADOWMAIL",
      "TROPICMAIL",    "DREAMMAIL",    "FABMAIL",      "RETROMAIL",
      "CHERIBERRY",    "CHESTOBERRY",  "PECHABERRY",   "RAWSTBERRY",
      "ASPEARBERRY",   "LEPPABERRY",   "ORANBERRY",    "PERSIMBERRY",
      "LUMBERRY",      "SITRUSBERRY",  "FIGYBERRY",    "WIKIBERRY",
      "MAGOBERRY",     "AGUAVBERRY",   "IAPAPABERRY",  "RAZZBERRY",
      "BLUKBERRY",     "NANABBERRY",   "WEPEARBERRY",  "PINAPBERRY",
      "POMEGBERRY",    "KELPSYBERRY",  "QUALOTBERRY",  "HONDEWBERRY",
      "GREPABERRY",    "TAMATOBERRY",  "CORNNBERRY",   "MAGOSTBERRY",
      "RABUTABERRY",   "NOMELBERRY",   "SPELONBERRY",  "PAMTREBERRY",
      "WATMELBERRY",   "DURINBERRY",   "BELUEBERRY",   "LIECHIBERRY",
      "GANLONBERRY",   "SALACBERRY",   "PETAYABERRY",  "APICOTBERRY",
      "LANSATBERRY",   "STARFBERRY",   "ENIGMABERRY",  "BRIGHTPOWDER",
      "WHITEHERB",     "MACHOBRACE",   "EXPSHARE",     "QUICKCLAW",
      "SOOTHEBELL",    "MENTALHERB",   "CHOICEBAND",   "KINGSROCK",
      "SILVERPOWDER",  "AMULETCOIN",   "CLEANSETAG",   "SOULDEW",
      "DEEPSEATOOTH",  "DEEPSEASCALE", "SMOKEBALL",    "EVERSTONE",
      "FOCUSBAND",     "LUCKYEGG",     "SCOPELENS",    "METALCOAT",
      "LEFTOVERS",     "DRAGONSCALE",  "LIGHTBALL",    "SOFTSAND",
      "HARDSTONE",     "MIRACLESEED",  "BLACKGLASSES", "BLACKBELT",
      "MAGNET",        "MYSTICWATER",  "SHARPBEAK",    "POISONBARB",
      "NEVERMELTICE",  "SPELLTAG",     "TWISTEDSPOON", "CHARCOAL",
      "DRAGONFANG",    "SILKSCARF",    "UPGRADE",      "SHELLBELL",
      "SEAINCENSE",    "LAXINCENSE",   "LUCKYPUNCH",   "METALPOWDER",
      "THICKCLUB",     "STICK",        "REDSCARF",     "BLUESCARF",
      "PINKSCARF",     "GREENSCARF",   "YELLOWSCARF",  "MACHBIKE",
      "COINCASE",      "ITEMFINDER",   "OLDROD",       "GOODROD",
      "SUPERROD",      "SSTICKET",     "CONTESTPASS",  "WAILMERPAIL",
      "DEVONGOODS",    "SOOTSACK",     "BASEMENTKEY",  "ACROBIKE",
      "POKEBLOCKCASE", "LETTER",       "EONTICKET",    "REDORB",
      "BLUEORB",       "SCANNER",      "GOGOGGLES",    "METEORITE",
      "RM1KEY",        "RM2KEY",       "RM4KEY",       "RM6KEY",
      "STORAGEKEY",    "ROOTFOSSIL",   "CLAWFOSSIL",   "DEVONSCOPE",
      "TM01",          "TM02",         "TM03",         "TM04",
      "TM05",          "TM06",         "TM07",         "TM08",
      "TM09",          "TM10",         "TM11",         "TM12",
      "TM13",          "TM14",         "TM15",         "TM16",
      ...
      "TM49",          "TM50",         "HM01",         "HM02",
      "HM03",          "HM04",         "HM05",         "HM06",
      "HM07",          "HM08",         "OAKSPARCEL",   "POKEFLUTE",
      "SECRETKEY",     "BIKEVOUCHER",  "GOLDTEETH",    "OLDAMBER",
      "CARDKEY",       "LIFTKEY",      "HELIXFOSSIL",  "DOMEFOSSIL",
      "SILPHSCOPE",    "BICYCLE",      "TOWNMAP",      "VSSEEKER",
      "FAMECHECKER",   "TMCASE",       "BERRYPOUCH",   "TEACHYTV",
      "TRIPASS",       "RAINBOWPASS",  "TEA",          "MYSTICTICKET",
      "AURORATICKET",  "POWDERJAR",    "RUBY",         "SAPPHIRE",
      "MAGMAEMBLEM",   "OLDSEAMAP"};
...
void FillMsgs(HWND hwndList) {
  char *pVarName[] = {"MSG_ITEM", "MSG_NORMAL",   "MSG_YESNO", "MSG_NOCLOSE",
                      "MSG_SIGN", "MSG_STANDARD", "MSG_FIND",  "MSG_OBTAIN"};
  SendMessage(hwndList, CB_RESETCONTENT, 0, 0);
  for (size_t i = 0; i < sizeof(pVarName) / sizeof(pVarName[0]); i++)
    SendMessage(hwndList, CB_ADDSTRING, 0, (LPARAM)pVarName[i]);
}
...
void FillPokemon(HWND hwndList) {
  char *pVarName[] = {
      ...
      "HO-OH",      "CELEBI",     "TREECKO",    "GROVYLE",    "SCEPTILE",
      ...
      "SHIFTRY",    "NINCADA",    "NINJASK",    "SHEDINJA",   "TAILLOW",
      "SWELLOW",    "SHROOMISH",  "BRELOOM",    "SPINDA",     "WINGULL",
      "PELIPPER",   "SURSKIT",    "MASQUERAIN", "WAILMER",    "WAILORD",
      "SKITTY",     "DELCATTY",   "KECLEON",    "BALTOY",     "CLAYDOL",
      "NOSEPASS",   "TORKOAL",    "SABLEYE",    "BARBOACH",   "WHISCASH",
      "LUVDISC",    "CORPHISH",   "CRAWDAUNT",  "FEEBAS",     "MILOTIC",
      "CARVANHA",   "SHARPEDO",   "TRAPINCH",   "VIBRAVA",    "FLYGON",
      "MAKUHITA",   "HARIYAMA",   "ELECTRIKE",  "MANECTRIC",  "NUMEL",
      "CAMERUPT",   "SPHEAL",     "SEALEO",     "WALREIN",    "CACNEA",
      "CACTURNE",   "SNORUNT",    "GLALIE",     "LUNATONE",   "SOLROCK",
      "AZURILL",    "SPOINK",     "GRUMPIG",    "PLUSLE",     "MINUN",
      "MAWILE",     "MEDITITE",   "MEDICHAM",   "SWABLU",     "ALTARIA",
      "WYNAUT",     "DUSKULL",    "DUSCLOPS",   "ROSELIA",    "SLAKOTH",
      "VIGOROTH",   "SLAKING",    "GULPIN",     "SWALOT",     "TROPIUS",
      "WHISMUR",    "LOUDRED",    "EXPLOUD",    "CLAMPERL",   "HUNTAIL",
      "GOREBYSS",   "ABSOL",      "SHUPPET",    "BANETTE",    "SEVIPER",
      "ZANGOOSE",   "RELICANTH",  "ARON",       "LAIRON",     "AGGRON",
      "CASTFORM",   "VOLBEAT",    "ILLUMISE",   "LILEEP",     "CRADILY",
      "ANORITH",    "ARMALDO",    "RALTS",      "KIRLIA",     "GARDEVOIR",
      "BAGON",      "SHELGON",    "SALAMENCE",  "BELDUM",     "METANG",
      "METAGROSS",  "REGIROCK",   "REGICE",     "REGISTEEL",  "KYOGRE",
      "GROUDON",    "RAYQUAZA",   "LATIAS",     "LATIOS",     "JIRACHI",
      "DEOXYS",     "CHIMECHO"};
  SendMessage(hwndList, CB_RESETCONTENT, 0, 0);
  for (size_t i = 0; i < sizeof(pVarName) / sizeof(pVarName[0]); i++)
    SendMessage(hwndList, CB_ADDSTRING, 0, (LPARAM)pVarName[i]);
...
void FillAttacks(HWND hwndList) {
  char *pVarName[] = {
      ...
      "MEGAPUNCH",    "PAYDAY",       "FIREPUNCH",    "ICEPUNCH",
      "THUNDERPUNCH", "SCRATCH",      "VICEGRIP",     "GUILLOTINE",
      "RAZORWIND",    "SWORDSDANCE",  "CUT",          "GUST",
      "WINGATTACK",   "WHIRLWIND",    "FLY",          "BIND",
      "SLAM",         "VINEWHIP",     "STOMP",        "DOUBLEKICK",
      "MEGAKICK",     "JUMPKICK",     "ROLLINGKICK",  "SANDATTACK",
      "HEADBUTT",     "HORNATTACK",   "FURYATTACK",   "HORNDRILL",
      "TACKLE",       "BODYSLAM",     "WRAP",         "TAKEDOWN",
      "THRASH",       "DOUBLEEDGE",   "TAILWHIP",     "POISONSTING",
      "TWINEEDLE",    "PINMISSLE",    "LEER",         "BITE",
      "GROWL",        "ROAR",         "SING",         "SUPERSONIC",
      "SONICBOOM",    "DISABLE",      "ACID",         "EMBER",
      "FLAMETHROWER", "MIST",         "WATERGUN",     "HYDROPUMP",
      "SURF",         "ICEBEAM",      "BLIZZARD",     "PSYBEAM",
      "BUBBLEBEAM",   "AURORABEAM",   "HYPERBEAM",    "PECK",
      "DRILLPECK",    "SUBMISSION",   "LOWKICK",      "COUNTER",
      "SEISMICTOSS",  "STRENGTH",     "ABSORB",       "MEGADRAIN",
      "LEECHSEED",    "GROWTH",       "RAZORLEAF",    "SOLARBEAM",
      "POISONPOWDER", "STUNSPORE",    "SLEEPPOWDER",  "PETALDANCE",
      "STRINGSHOT",   "DRAGONRAGE",   "FIRESPIN",     "THUNDERSHOCK",
      "THUNDERBOLT",  "THUNDERWAVE",  "THUNDER",      "ROCKTHROW",
      "EARTHQUAKE",   "FISSURE",      "DIG",          "TOXIC",
      "CONFUSION",    "PSYCHIC",      "HYPNOSIS",     "MEDITATE",
      "AGILITY",      "QUICKATTACK",  "RAGE",         "TELEPORT",
      "NIGHTSHADE",   "MIMIC",        "SCREECH",      "DOUBLETEAM",
      "RECOVER",      "HARDEN",       "MINIMIZE",     "SMOKESCREEN",
      "CONFUSERAY",   "WITHDRAW",     "DEFENSECURL",  "BARRIER",
      "LIGHTSCREEN",  "HAZE",         "REFLECT",      "FOCUSENERGY",
      "BIDE",         "METRONOME",    "MIRRORMOVE",   "SELFDESTRUCT",
      "EGGBOMB",      "LICK",         "SMOG",         "SLUDGE",
      "BONECLUB",     "FIREBLAST",    "WATERFALL",    "CLAMP",
      "SWIFT",        "SKULLBASH",    "SPIKECANNON",  "CONSTRICT",
      "AMNESIA",      "KINESIS",      "SOFTBOILED",   "HIJUMPKICK",
      "GLARE",        "DREAMEATER",   "POISONGAS",    "BARRAGE",
      "LEECHLIFE",    "LOVELYKISS",   "SKYATTACK",    "TRANSFORM",
      "BUBBLE",       "DIZZYPUNCH",   "SPORE",        "FLASH",
      "PSYWAVE",      "SPLASH",       "ACIDARMOR",    "CRABHAMMER",
      "EXPLOSION",    "FURYSWIPES",   "BONEMERANG",   "REST",
      "ROCKSLIDE",    "HYPERFANG",    "SHARPEN",      "CONVERSION",
      "TRIATTACK",    "SUPERFANG",    "SLASH",        "SUBSTITUTE",
      "STRUGGLE",     "SKETCH",       "TRIPLEKICK",   "THIEF",
      "SPIDERWEB",    "MINDREADER",   "NIGHTMARE",    "FLAMEWHEEL",
      "SNORE",        "CURSE",        "FLAIL",        "CONVERSION2",
      "AEROBLAST",    "COTTONSPORE",  "REVERSAL",     "SPITE",
      "POWDERSNOW",   "PROTECT",      "MACHPUNCH",    "SCARYFACE",
      "FAINTATTACK",  "SWEETKISS",    "BELLYDRUM",    "SLUDGEBOMB",
      "MUDSLAP",      "OCTAZOOKA",    "SPIKES",       "ZAPCANNON",
      "FORESIGHT",    "DESTINYBOND",  "PERISHSONG",   "ICYWIND",
      "DETECT",       "BONERUSH",     "LOCKON",       "OUTRAGE",
      "SANDSTORM",    "GIGADRAIN",    "ENDURE",       "CHARM",
      "ROLLOUT",      "FALSESWIPE",   "SWAGGER",      "MILKDRINK",
      "SPARK",        "FURYCUTTER",   "STEELWING",    "MEANLOOK",
      "ATTRACT",      "SLEEPTALK",    "HEALBELL",     "RETURN",
      "PRESENT",      "FRUSTRATION",  "SAFEGUARD",    "PAINSPLIT",
      "SACREDFIRE",   "MAGNITUTDE",   "DYNAMICPUNCH", "MEGAHORN",
      "DRAGONBREATH", "BATONPASS",    "ENCORE",       "PURSUIT",
      "RAPIDSPIN",    "SWEETSCENT",   "IRONTAIL",     "METALCLAW",
      "VITALTHROW",   "MORNINGSUN",   "SYNTHESIS",    "MOONLIGHT",
      "HIDDENPOWER",  "CROSSCHOP",    "TWISTER",      "RAINDANCE",
      "SUNNYDAY",     "CRUNCH",       "MIRRORCOAT",   "PSYCHUP",
      "EXTREMESPEED", "ANCIENTPOWER", "SHADOWBALL",   "FUTURESIGHT",
      "ROCKSMASH",    "WHIRLPOOL",    "BEATUP",       "FAKEOUT",
      "UPROAR",       "STOCKPILE",    "SPITUP",       "SWALLOW",
      "HEATWAVE",     "HAIL",         "TORMENT",      "FLATTER",
      "WILLOWISP",    "MEMENTO",      "FACADE",       "FOCUSPUNCH",
      "SMELLINGSALT", "FOLLOWME",     "NATUREPOWER",  "CHARGE",
      "TAUNT",        "HELPINGHAND",  "TRICK",        "ROLEPLAY",
      "WISH",         "ASSIST",       "INGRAIN",      "SUPERPOWER",
      "MAGICCOAT",    "RECYCLE",      "REVENGE",      "BRICKBREAK",
      "YAWN",         "KNOCKOFF",     "ENDEAVOUR",    "ERUPTION",
      "SKILLSWAP",    "IMPRISON",     "REFRESH",      "GRUDGE",
      "SNATCH",       "SECRETPOWER",  "DIVE",         "ARMTHRUST",
      "CAMOUFLAGE",   "TAILGLOW",     "LUSTERPURGE",  "MISTBALL",
      "FEATHERDANCE", "TEETERDANCE",  "BLAZEKICK",    "MUDSPORT",
      "ICEBALL",      "NEEDLEARM",    "SLACKOFF",     "HYPERVOICE",
      "POISONFANG",   "CRUSHCLAW",    "BLASTBURN",    "HYDROCANNON",
      "METEORMASH",   "ASTONISH",     "WEATHERBALL",  "AROMATHERAPY",
      "FAKETEARS",    "AIRCUTTER",    "OVERHEAT",     "ODORSLEUTH",
      "ROCKTOMB",     "SILVERWIND",   "METALSOUND",   "GRASSWHISTLE",
      "TICKLE",       "COSMICPOWER",  "WATERSPOUT",   "SIGNALBEAM",
      "SHADOWPUNCH",  "EXTRASENSORY", "SKYUPPERCUT",  "SANDTOMB",
      "SHEERCOLD",    "MUDDYWATER",   "BULLETSEED",   "AERIALACE",
      "ICICLESPEAR",  "IRONDEFENSE",  "BLOCK",        "HOWL",
      "DRAGONCLAW",   "FRENZYPLANT",  "BULKUP",       "BOUNCE",
      "MUDSHOT",      "POISONTAIL",   "COVET",        "VOLTTACKLE",
      "MAGICALLEAF",  "WATERSPORT",   "CALMMIND",     "LEAFBLADE",
      "DRAGONDANCE",  "ROCKBLAST",    "SHOCKWAVE",    "WATERPULSE",
      "DOOMDESIRE",   "PSYCHOBOOST"};
  SendMessage(hwndList, CB_RESETCONTENT, 0, 0);
  for (size_t i = 0; i < sizeof(pVarName) / sizeof(pVarName[0]); i++)
    SendMessage(hwndList, CB_ADDSTRING, 0, (LPARAM)pVarName[i]);
}
...
DWORD WINAPI HMMoveScript(LPVOID arg1) {
  ...
  if (!cancelled) {
    ...
    sprintf(
        ...
        "0x81BDFD7\ncallstd MSG_NOCLOSE\nclosemsg\ndoanimation "
        "2\nwaitspecial\n' -------------------\n' Enter the code here for what "
        "happens when the attack is used\n\n\n' "
        "-------------------\nreleaseall\nend\n\n#org @donthave\nmsgbox "
        "@donthavemsg\ncallstd 0x3\nreleaseall\nend\n\n#org @doit\n= Do you "
        "want to use that attack%cnon this[.] thing? Replace this message with "
        "something else. Please.\n\n#org @donthavemsg\n= You don't have the "
        "correct attack. Also replace this message.\n",
      ...
      if (!cancelled) {
        ...
        if (!cancelled) {
          ...
          sprintf(
              ...
              "\\v\\h03s.\n\n#org @msg-have\n= Do you want to craft one "
              "\\v\\h02\\nwith %s \\v\\h03s?\n\n#org @finished\n= Done! You "
              "used %s \\v\\h03s\\nto create a \\v\\h02!\n",
              GreatBuffer, buf2, buf1, buf2, buf3, buf2, buf3, GreatBuffer,
              buf1, buf3, buf3, buf3);
          SendEditor(SCI_CANCEL, 0, 0);
          SendEditor(SCI_SETUNDOCOLLECTION, 0, 0);
          SendEditor(SCI_EMPTYUNDOBUFFER, 0, 0);
        ...
        if (!cancelled) {
          ...
          if (!cancelled) {
            ...
            if (!cancelled) {
              ...
              GetInput2(GetString1(3077), GetString2(3084), GreatBuffer,
                        FillItems);
              while (inputrunning) {
                Sleep(0);
              }
              if (!cancelled) {
                ...
                if (!cancelled) {
                  ...
                  SendEditor(SCI_MARKERDELETEALL, (WPARAM)-1, 0);
                  SendEditor(SCI_SETSCROLLWIDTH, 2048, 0);
                  SendEditor(SCI_ADDTEXT, strlen(MegaBuffer),
                             (LPARAM)MegaBuffer);
                  SendEditor(SCI_SETUNDOCOLLECTION, 1, 0);
                  SendEditor(EM_EMPTYUNDOBUFFER, 0, 0);
                  SendEditor(SCI_SETSAVEPOINT, 0, 0);
...
DWORD WINAPI RockBlockScript(LPVOID arg1) {
  ...
  if (!cancelled) {
    ...
    sprintf(
        ...
        "@havemsg ' This rock appears to...\ncallstd MSG_YESNO ' Yes/No "
        "message\ncompare LASTRESULT 0\nif == jump :end\nremoveitem %s "
...
DWORD WINAPI FlagSkeleton2Script(LPVOID arg1) {
  ...
  if (!cancelled) {
    ...
    sprintf(
        ...
        "jump :flag-2\n\n//Flag has not been activated\nmsgbox "
        "@flag-1-msg\ncallstd MSG_NOCLOSE\n\n\nsetflag %s\n:flag-2\n\n//Always "
        "executed whether flag is set or not\nmsgbox @flag-2-msg\ncallstd "
        "MSG_NOCLOSE\n\n\nrelease\nend\n\n#org @flag-1-msg\n= Hello, I'm "
        "Jekyll.\n\n#org @flag-2-msg\n= Well, goodbye.",
        GreatBuffer, GreatBuffer);
    SendEditor(SCI_CANCEL, 0, 0);
    ...
    case WM_COMMAND:
      switch (LOWORD(wParam)) {
        ...
        case 0x13:
          ...
          break;
        case 0x5:
          ShellExecute(GenWin, NULL,
                       "mailto:seejay.11@gmail.com?subject=PKSVUI&body=Hi,%20I%"
                       "20Would%20like...",
        ...
        case 0x6:
          ShellExecute(
              GenWin, NULL,
              "http://www.pokecommunity.com/private.php?do=newpm&u=15354", NULL,
              NULL, SW_SHOWDEFAULT);
          break;
        case 0x1:
        case 2000:
...
void SwapItems(unsigned int a, unsigned int b) {
  ...
  list *f = moves;
  unsigned int i;
  for (i = 0; i <= a && f; i++)  // loop until end of moves, or until the larger
                                 // of a or b is reached.
  {
    if (i == a) c = f;
    f = f->next;
  }
...
BOOL CALLBACK MovePlanFunc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
  ...
  switch (msg) {
    case WM_DRAWITEM:
      ...
      x = (((((LPDRAWITEMSTRUCT)lParam)->rcItem.right) -
            (((LPDRAWITEMSTRUCT)lParam)->rcItem.left)) /
           2) -
          8;
      y = (((((LPDRAWITEMSTRUCT)lParam)->rcItem.bottom) -
            (((LPDRAWITEMSTRUCT)lParam)->rcItem.top)) /
           2) -
          8;
      i = x & 15;
      j = ((LPDRAWITEMSTRUCT)lParam)->rcItem.bottom -
          ((LPDRAWITEMSTRUCT)lParam)->rcItem.top;
      ...
      i = y & 15;
      j = ((LPDRAWITEMSTRUCT)lParam)->rcItem.right -
          ((LPDRAWITEMSTRUCT)lParam)->rcItem.left;
      ...
      while (c) {
        ...
        if (i >= 1 && i <= 4) {
          ...
          BitBlt(dc, x + 3, y + 3, 11, 11, dc2, ((i - 2) & 3) * 11 + 152, 11,
                 SRCAND);
          BitBlt(dc, x + 3, y + 3, 11, 11, dc2, ((i - 2) & 3) * 11 + 152, 0,
                 SRCPAINT);
          ...
          BitBlt(dc, x + 1, y + 1, 16, 16, dc2, ((i - 2) & 3) * 16 + 120, 38,
                 SRCAND);
          BitBlt(dc, x + 1, y + 1, 16, 16, dc2, ((i - 2) & 3) * 16 + 120, 22,
                 SRCPAINT);
          if ((i & 3) == 2) {
            y += 8;
          } else if ((i & 3) == 3) {
          ...
          BitBlt(dc, x + 3, y + 3, 11, 11, dc2, ((i - 1) & 3) * 11 + 108, 11,
                 SRCAND);
          BitBlt(dc, x + 3, y + 3, 11, 11, dc2, ((i - 1) & 3) * 11 + 108, 0,
                 SRCPAINT);
          if ((i & 3) == 1) {
            y += 8;
          } else if ((i & 3) == 2) {
    ...
    case WM_COMMAND:
      ...
      switch (LOWORD(wParam)) {
        ...
        case 8:
          ...
          if (i != 0 && i != (unsigned)LB_ERR) SwapItems(i - 1, i - 1);
          break;
        case 9:
          i = SendMessage(GetDlgItem(MovePlanWin, 13), LB_GETCURSEL, 0, 0);
          if (i != (unsigned)LB_ERR) SwapItems(i, i + 1);
          break;
        case 100:
          txttype = "Look";
          currtype = 0x0;
        ...
        case 107:
          ...
          break;
        case 10:
          i = (unsigned int)SendMessage(GetDlgItem(MovePlanWin, 13),
                                        LB_GETCURSEL, 0, 0);
          if (i == (unsigned)LB_ERR) break;
          SendMessage(GetDlgItem(MovePlanWin, 13), LB_DELETESTRING, i, 0);
          if (SendMessage(GetDlgItem(MovePlanWin, 13), LB_SETCURSEL, i, 0) ==
              LB_ERR)
    ...
    case WM_COMMAND:
      switch (LOWORD(wParam)) {
        ...
        case 3:
          ...
          if (FlagArr[LastFlag] > x) LastFlag++;
          SortFlags();
          for (i = 0; i < FlagArrLen; i++)
            if (FlagArr[i] == x) {
        ...
        case 11:
          ...
          break;
        case 8:
          if (MessageBox(FlagsWin, GetString1(3092), GetString1(3091),
                         0x20 | MB_YESNO) == 6) {
    ...
    case WM_COMMAND:
      switch (LOWORD(wParam)) {
        case 1:
          ...
          break;
        case 3:
          txtlen = SendMessage(GetDlgItem(ScriptSearchWin, 2), WM_GETTEXTLENGTH,
                               0, 0);
          mem = GlobalAlloc(GPTR, txtlen + 100);
          strcpy(mem, "#org 0x0\n");
          SendMessage(GetDlgItem(ScriptSearchWin, 2), WM_GETTEXT, txtlen + 1,
                      (LPARAM)mem + strlen("#org 0x0\n"));
          CloseHandle(CreateFile("tmp.bin", GENERIC_WRITE, 0, NULL,
        ...
        case 5:
          ...
          if (i == -1) sscanf(buf, "%X", (unsigned int *)&i);
          if (i == -1) sscanf(buf, "$%X", (unsigned int *)&i);
          binlen = ReadWholeFile("tmp.bin", &mem);
          if (!romloaded) romlen = ReadWholeFile(RomOpen2, &mem2);
          romloaded = 1;
          if (!mem || !mem2) {
            MessageBox(ScriptSearchWin, GetString1(3088), GetString2(3095),
                       0x10);
          ...
          if (IsDlgButtonChecked(ScriptSearchWin, 24)) binlen--;
          j = 0;
          for (i++; i < romlen; i++) {
            for (j = 0; j < binlen; j++) {
              if (mem[j] != mem2[i] &&
                  (unsigned char)mem[j] != (unsigned char)0xBA) {
                break;
              }
              i++;
            }
        ...
        case 9:
          ...
          break;
        case 16:
          i = SendMessage(GetDlgItem(ScriptSearchWin, 14), WM_GETTEXTLENGTH, 0,
                          0);
  ...
  switch (msg) {
    case WM_COMMAND:
      switch (LOWORD(wParam)) {
        ...
        case 10:
          file =
              CreateFile(RomOpen2, GENERIC_WRITE,
                         FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE,
        ...
        case 8:
          file =
              CreateFile(RomOpen2, GENERIC_READ,
                         FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE,
                         NULL, OPEN_EXISTING, 0, NULL);
          if (file == INVALID_HANDLE_VALUE) {
            MessageBox(LockWin, GetString1(3096), GetString2(3001), 0x10);
  ...
  switch (msg) {
    case WM_NOTIFY:
      switch (LOWORD(wParam)) {
        case 1000:
          switch (
              SendMessage(GetDlgItem(PickupWin, 1000), TCM_GETCURSEL, 0, 0)) {
            case 0:
              for (i = 1; i <= 10; i++)
                ShowWindow(GetDlgItem(PickupWin, i), SW_SHOW);
              for (i = 50; i <= 87; i++)
                ShowWindow(GetDlgItem(PickupWin, i), SW_HIDE);
              break;
            case 1:
              for (i = 50; i <= 87; i++)
                ShowWindow(GetDlgItem(PickupWin, i), SW_SHOW);
              for (i = 1; i <= 10; i++)
                if (i != 2) ShowWindow(GetDlgItem(PickupWin, i), SW_HIDE);
              break;
          }
          break;
      }
    ...
    case WM_COMMAND:
      switch (LOWORD(wParam)) {
        case 52:
          ...
          if (start == -1) break;
          file =
              CreateFile(RomOpen2, GENERIC_READ | GENERIC_WRITE,
                         FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE,
                         NULL, OPEN_EXISTING, 0, NULL);
          if (file == INVALID_HANDLE_VALUE) {
          ...
          if (modechar == 'ERPB')
            SetFilePointer(file, 0x3DB028 + 0x2C * start, NULL, FILE_BEGIN);
          else if (modechar == 'EGPB')
            SetFilePointer(file, 0x3DAE64 + 0x2C * start, NULL, FILE_BEGIN);
          else {
            CloseHandle(file);
        ...
        case 50:
          if (HIWORD(wParam) == CBN_SELCHANGE) {
            ...
            for (j = 0; j < 5; j++)
              if (i == j + 1)
                CheckDlgButton(PickupWin, 82 + j, 1);
              else
                CheckDlgButton(PickupWin, 82 + j, 0);
            if (i == 0 || i > 5) CheckDlgButton(PickupWin, 87, 1);
            ReadFile(file, &i, 0x1, &read, NULL);
            sprintf(buf2, "%u", i);
        ...
        case 5:
          if (HIWORD(wParam) == LBN_SELCHANGE) {
            ...
            sprintf(buf, "%u",
                    *(unsigned short *)(PickupBuf + 2 + (start << 2)));
        ...
        case 3:
          if (HIWORD(wParam) == CBN_SELCHANGE) {
            ...
            if (start != -1) {
              ...
              SendMessage(GetDlgItem(PickupWin, 5), LB_ADDSTRING, 0,
                          (LPARAM)buf);
        ...
        case 6:
          ...
          if (SendMessage(GetDlgItem(PickupWin, 1000), TCM_GETCURSEL, 0, 0) ==
              0) {
