...
void bsearch_init_root(struct bsearch_root *root, bsearch_compare_func *compare, bsearch_copy_func *copy, bsearch_free_key_func *free_key, bsearch_free_val_func *free_val) {
  size_t capacity = 16;
...
struct bsearch_root* bsearch_create_root(bsearch_compare_func *compare, bsearch_copy_func *copy, bsearch_free_key_func *free_key, bsearch_free_val_func *free_val) {
...
void bsearch_deinit_root(struct bsearch_root* restrict root) {
  for (size_t i = 0; i < root->size; i++) {
    if (root->free_key)
      root->free_key((void*)root->pairs[i].key);
    if (root->free_val)
      root->free_val(root->pairs[i].value);
  }
  free(root->pairs);
}
...
static size_t bsearch_find(struct bsearch_kv const *kvs, size_t len, void const *key, bsearch_compare_func *compare) {
  size_t left = 0;
  size_t right = len;
  while (left < right) {
...
size_t bsearch_upsert(struct bsearch_root *kvs, void const *key, void *value) {
  ...
  if (index < len && kvs->compare(kvs->pairs[index].key, key) == 0) {
    ...
    if (kvs->free_val)
      kvs->free_val(kvs->pairs[index].value);
