...
#define GLD_GIVEPOKE \
  0x2D  // C,D poke(1) levl(1) item(1) trnr(1) if(trnr==1){trname(2) pknick(2)}
#define GLD_GIVEEGG 0x2E  // C,D poke(1) levl(1)
#define GLD_GIVEPOKEITEM \
  0x2F  // C,D itm*(2) ptr= itm#(1) + "THIS IS MESSAGE TEXT" (used in letters)
...
#define GLD_FACEPERSON \
  0x6B  // C,D targetperson(1) facingperson(1) if[tprs>fd]tprs=talkedto,
        // if[fprs=fe]tprs=talkedto
...
#define CRY_FACEPERSON \
  0x6C  // C,D targetperson(1) facingperson(1) if[tprs>fd]tprs=talkedto,
        // if[fprs=fe]tprs=talkedto
