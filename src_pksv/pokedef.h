...
//   cleaner program. 0x8004 is number in party. (eg. 0 is the 1st poke, 1 is
//   2nd etc)
...
#define CMD_WARPWALKING \
  0x3B  // C,D,T bank map exit - ALL 1 BYTE FRONLY:x(2) y(2)
#define CMD_FALLDOWNHOLE 0x3C  // C,D,T bank map
#define CMD_WARPTELEPORT \
  0x3D                  // C,D,T bank map exit - ALL 1 BYTE FRONLY:x(2) y(2)
#define CMD_WARP3 0x3E  // C,D,T bank map exit - ALL 1 BYTE FRONLY:x(2) y(2)
#define CMD_WARPELEVATOR \
  0x3F                  // C,D,T bank map exit - ALL 1 BYTE FRONLY:x(2) y(2)
...
#define CMD_TRAINERBATTLE \
  0x5C  // C,D,T kind[0x0==normal,0x3==rival/elite4,0x4==double,0x5==rematch](1)
        // num(2) ???(2) start(4) defeat(4)
...
#define CMD_MULTICHOICE \
  0x6F  // C,D,T x(1) y(1) list(1) AbleToCancel(1) OR'd binary bits. 0x1 =
        // unable, 0x2 = return 127
#define CMD_MULTICHOICE2 \
  0x70  // C,D,T x(1) y(1) list(1) defchoice(1) AbleToCancel(1)
...
if ?? jump ???????? Checks previously compared values, and might jump, depending
on what they are. (That was /so/ hard to explain) if ?? call ???????? same as
the jumping if, except it can be returned to jumpstd is like "boxset", except it
ends the script too. callstd is "boxset", and is returned to when it finishes.
killscript I'm not sure what this does, but it clears some stuff to do with the
script. setbyte I'm not sure what this is used for, but it sets a byte
[somewhere in RAM-find out] to a value. (You could use it for callasm) msgbox
Loads a message into memory, ready to be displayed by callstd. loadpointer Loads
anything into memory, for various uses. writebytetooffset writes a byte to an
address in memory. loadbytefrompointer ...er - not sure copyscriptbanks Copies a
bank of scripts from one bank to another. God knows what you could use this for.
...
Copyvarifnotzero Copies one variable to another, if the value is not zero, else
it does nothing. ( if(b>0)a=b; ) comparevartobyte Compares a variable to a byte.
( var(a)==b ) comparevartofarbyte Compares a variable to a byte at a certain
...
storetext Stores text (argument 2) into text-var (\v\hXX+2) where XX is
argument 1. pokemart Opens up a pokemon-mart screen. slotmachine Whee, fun! It's
the slots! fadescreen Fades in or out.
cmd_91 ¯\
cmd_92  |
cmd_93  |
...
setmaptile Sets a tile on the map to something different. May be used for making
a tile walkable-across. setweather Sets a new weather (snow :oD ) doweather
Applies the weather change coincasetovar Copies the amount of coins in the coin
case (even if you don't have one) to a variable. givetocoincase Gives money to a
coin-case, even if you don't have one. battle Stores the variables necessary for
having a wild pokemon battle.
